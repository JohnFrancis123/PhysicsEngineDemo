...
string FileController::GetCurDirectory() {
	...
	M_ASSERT(GetCurrentDir(buff, FILENAME_MAX) != nullptr, "Could not get current directory.");//this is what happens when we get a nullpointer
	return string(buff);
}

int FileController::GetFileSize(string _filePath) { //c_str is the same thing. 
	m_handle = nullptr;
	//cout << &m_handle;
	//cout << m_handle;
	M_ASSERT(fopen_s(&m_handle, _filePath.c_str(), "rb") == 0, "Could not open file."); //Error here
	if (m_handle != nullptr) {
