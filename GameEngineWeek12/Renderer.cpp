...
//NOTE: SDL_RENDERER_PRESENTVSYNC will cap framerate at your monitor refresh rate.
void Renderer::Initialize() {
	...
	m_window = SDL_CreateWindow("SDL Window", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
		m_srcRect.w, m_srcRect.h, SDL_WINDOW_FULLSCREEN_DESKTOP); //SDL_WINDOW_FULLSCREEN. Also h was y. we changed in the lecture. Changed to SDL_WINDOW_FULLSCREEN_DESKTOP as of assignment 2
...
SDL_Texture* Renderer::GetSDLTexture(Texture* _texture) {
	...
	if (m_textures.count(guid) == 0) { //remember we are using a map(dictionary) in the VRAM
...
{ //required to pass by value here. Otherwise, it will be deleted.
	int w;
	int h;
	SDL_GetWindowSize(m_window, &w, &h);
	return Point(w, h); //creating value on stack and passing copy to calling function
}
...
void Renderer::SetViewport(Rect _viewport) {
	...
	SDL_RenderSetViewport(m_renderer, &m_viewPort); //&m_viewport acts as a Rectangle object pointer.
} //we RenderSetViewport with a reference/pointer
